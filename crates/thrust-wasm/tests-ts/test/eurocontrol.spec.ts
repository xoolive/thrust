...
function assertCoreEurocontrolEntities(
  resolver: Resolver,
  label: string,
  options: { checkAirportNames?: boolean } = {},
): void {
  ...
  if (options.checkAirportNames) {
    ...
    if (!eham || (!ehamName.includes("SCHIPHOL") && !ehamName.includes("AMSTERDAM"))) {
      throw new Error(`[${label}] Unexpected EHAM airport name: ${String(eham?.name || "")}`);
    }

    const lszh = resolver.resolve_airport("LSZH") as { name?: string } | null;
    if (!lszh || !String(lszh.name || "").toUpperCase().includes("ZURICH")) {
      throw new Error(`[${label}] Unexpected LSZH airport name: ${String(lszh?.name || "")}`);
    }
  }
  ...
  const gai = resolver.resolve_navaid("GAI") as { code?: string; description?: string } | null;
  if (!gai || String(gai.code || "").toUpperCase() !== "GAI") {
    throw new Error(`[${label}] Failed to resolve Eurocontrol navaid GAI`);
  }
  if (!String(gai.description || "").toUpperCase().includes("GAILLAC")) {
    throw new Error(`[${label}] Unexpected GAI navaid description: ${String(gai.description || "")}`);
  }

  const tou = resolver.resolve_navaid("TOU") as { code?: string; description?: string } | null;
  if (!tou || String(tou.code || "").toUpperCase() !== "TOU") {
    throw new Error(`[${label}] Failed to resolve Eurocontrol navaid TOU`);
  }
  if (!String(tou.description || "").toUpperCase().includes("TOULOUSE")) {
    throw new Error(`[${label}] Unexpected TOU navaid description: ${String(tou.description || "")}`);
  }

  const um605 = resolver.resolve_airway("UM605") as { name?: string; points?: unknown[] } | null;
